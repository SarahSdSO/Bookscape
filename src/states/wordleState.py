...
class WordleState(State):
    def __init__(self, game):
        ...
        self.board_x = (self.WIDTH - (5 * 73 + 74)) // 2
        ...
        for row in range(0, 6):
            if self.roundGame > row:
                ...
                if guess == self.secretWord:
                    for col in range(0, 5):
                        pygame.draw.rect(self.game.screen, self.green, 
                                        (self.board_x + col * 73 + 74, self.board_y + row * 76 + 60, 60, 60), 0, 8)
                else:
                    for col in range(0, 5):
                        if self.rowsBoard[row][col] == self.secretWord[col]:
                            ...
                            pygame.draw.rect(self.game.screen, self.yellow, 
                                            (self.board_x + col * 73 + 74, self.board_y + row * 76 + 60, 60, 60), 0, 8)
                        else:
                            pygame.draw.rect(self.game.screen, self.gray, 
                                            (self.board_x + col * 73 + 74, self.board_y + row * 76 + 60, 60, 60), 0, 8)
    ...
    def draw(self):
        ...
        for i in range(self.game.lives):
            self.game.screen.blit(self.game.heart_image, (self.WIDTH - (i + 1) * 60 - 10, 20))
